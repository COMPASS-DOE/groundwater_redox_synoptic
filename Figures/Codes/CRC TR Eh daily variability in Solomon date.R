...
# Graph Groundwater level below surface (m) vs time
df_troll %>% 
  filter(datetime>"2023-03-01 11:00:00", 
         datetime<"2023-04-30 11:00:00" ,
         #flag_out_of_water=="FALSE",
        # specific_conductivity>30,
         site=="CRC",
         location=="TR") %>%
  ggplot(aes(x = datetime, y = specific_conductivity, color=wl_below_surface_m))+
  geom_point(size = 3, alpha=0.3)+
  labs(title="CRC - TR", y="Specific Conductivity (uS/cm)", x="2023")+
  labs(color = "GW level (m)")

df_troll %>% 
  filter(datetime>"2023-04-01 11:00:00", 
         datetime<"2023-04-30 11:00:00" ,
         #flag_out_of_water=="FALSE",
         # specific_conductivity>30,
         site=="CRC",
  ...
  ggplot(aes(x = datetime, y = specific_conductivity, color=wl_below_surface_m))+
  geom_point(size = 3, alpha=0.3)+
  labs(title="CRC - TR", y="Specific Conductivity (uS/cm)", x="Time")+
  labs(color = "GW level (m)")




  ylab = expression(paste("Specific Conductivity ( ", mu, " )"))+
  theme(text = element_text(size = 15))+
  scale_x_datetime(limits = c(as.POSIXct("2022-08-25 00:00:00"), NA),
                   date_breaks = "2 days", 
                   date_labels = "%d/%m")
...
df_troll %>% 
  filter(datetime>"2022-08-25 11:00:00", 
  ...
  ggplot(aes(x = datetime, 
             y = Eh,
             color=wl_below_surface_m)
  )+
  geom_point(size = 3, 
             alpha=0.3
  )+
  labs(title="CRC - TR", y="Eh (mV)", x="Time")+
  labs(color = "GW level (m)")+
  theme(text = element_text(size = 15))+
  scale_x_datetime(limits = c(as.POSIXct("2022-08-25 00:00:00"), NA),
                   date_breaks = "2 days", 
...
pp4 <- p4+theme(plot.margin = unit(c(0.1, 1, 0.1, 0.5), "cm"))
pp5 <- p5+theme(plot.margin = unit(c(0.1, 1, 0.1, 0.5), "cm"))
pp6 <- p6+theme(plot.margin = unit(c(0.1, 1, 0.1, 0.5), "cm"))

ppp <- list(pp1, pp2, pp4, pp5, pp6)
