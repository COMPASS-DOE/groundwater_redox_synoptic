...
df %>% 
  ...
  ggplot(aes(x = datetime, y = detrended_data))+
  geom_point(size = 3, alpha=0.05, color="blue4")+
  labs(title="", y="GWL (m, detrended)", x="Time")+
  theme(text = element_text(size = 18))+
  guides(colour = guide_legend(override.aes = list(alpha = 1)))->p3
...
pp <- list(p1, p3, p4, p5)
plot_grid(plotlist=pp, nrow = 4, ncol=1, align='hv',
          rel_widths= c(1,1,1,1),rel_heights = c(1.2,1.2,1.2,1.2))
