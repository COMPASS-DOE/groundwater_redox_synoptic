...
df_troll %>%
  ...
  geom_point(aes(y = rdo_concen), color = "black", size = 0.1) +
  scale_y_continuous(name = "Greoundwater level (m)", 
                     sec.axis = sec_axis(~ . * 1, name = "DO (mg/L)")
...
ggsave("GWL_vs_datetime_Eh_color.png", plot = p, dpi = 300)
...
plots <- df_troll %>%
  filter(datetime > "2022-03-01 11:00:00",
         ...
         site != "GWI", site != "MSM") %>%
  split(.$site_location) %>%
  map(~ ggplot(data = .x, aes(x = datetime, y = wl_below_surface_m)) +
        geom_point(size = 3, alpha = 0.3) +
        labs(title = "", y = "Groundwater level (m)", x = "Time") +
        theme(text = element_text(size = 15)) +
...
plots_with_secondary_y <- imap(plots, function(plot, facet) {
  plot +
    geom_line(aes(y = Eh), color = "red", linewidth = 1.5) +
    scale_y_continuous(
      name = "Groundwater level (m)",
      sec.axis = sec_axis(~ . * 200, name = "Eh (mV)")
    ) +
    facet_wrap(~ facet, scales = "free_y", labeller = as_labeller(unique(df_troll$site_location)))
})

# Combine plots using patchwork
combined_plot <- wrap_plots(plots_with_secondary_y, nrow = 1)
combined_plot
