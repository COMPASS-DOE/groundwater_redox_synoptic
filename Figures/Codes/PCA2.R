...
## packages
require(pacman)
p_load(tidyverse, palmerpenguins, ggfortify, corrplot, paletteer, corrplot, psych, caret)
...
df_split <- split(df_std[, 5:23], df_std$site)
# Compute correlation matrix for each subset
corr_list <- lapply(df_split, cor)
# Combine correlation matrices into a single dataframe
corr_matrix <- do.call(rbind, corr_list)
...
corr_order <- order(as.numeric(factor(df_std$site))) # replace with your desired column for ordering
corr_matrix <- corr_matrix[corr_order, corr_order]

# Set threshold for significant correlation
corr_threshold <- 0.5

# Set color for insignificant correlation
corr_gray <- "gray"

# Visualize correlation matrix
corrplot(corr_matrix, method = "ellipse", order = "hclust", type = "lower", tl.col = ifelse(abs(corr_matrix) > corr_threshold, "black", corr_gray))
